...
import { pianni, projectorIj } from '../../src/c4OtimesC4/index.js';
...
describe('pianni', () => {
  ...
  it('has the expected diagonal', () => {
    ...
    for (let i = 0; i < 16; i++) {
      expect(real(rows[i][i]), `diagonal entry ${i}`).toBeCloseTo(heavy.has(i) ? 1 / 8 : 1 / 24, 10);
    }
  });
