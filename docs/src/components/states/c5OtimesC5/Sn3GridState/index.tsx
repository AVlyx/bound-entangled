...
const HYPEREDGES: ReadonlyArray<{ label: string; vertices: Hyperedge }> = [
  ...
  { label: "edge (1,2)–(3,4)", vertices: [[1, 2], [3, 4]] },
  { label: "edge (2,1)–(4,3)", vertices: [[2, 1], [4, 3]] },
  { label: "edge (2,2)–(3,3)", vertices: [[2, 2], [3, 3]] },
  { label: "hyperedge (0,2)-(1,1)-(2,0)", vertices: [[0, 2], [1, 1], [2, 0]] },
];
...
function Sn3GridState() {
  ...
  return (
    <>
      <p>
        The smallest known Schmidt number 3 PPT bound entangled state, on C⁵ ⊗ C⁵. It is an
        instance of the generalized quantum grid state construction: a mixture of pure states,
        each carried by a hyperedge over the product basis, chosen so the resulting state stays
        positive under partial transpose while requiring entangling operations of Schmidt rank at
        least 3 to prepare.
      </p>
      ...
      <div className="doc-section">
        ...
        <p>
          ...
          <span className="math">|i⟩ ⊗ |j⟩</span> of C<span className="math-var">m</span> ⊗
          C<span className="math-var">n</span>, and whose edges pair up vertices. An edge joining{" "}
          <span className="math">(i, j)</span> and <span className="math">(k, l)</span> carries
          the pure state <span className="math">(|ij⟩ − |kl⟩) / √2</span>, and the grid state is
          the uniform mixture <span className="math">ρ = (1 / |E|) Σₑ |e⟩⟨e|</span> of the states
          carried by every edge.
        </p>
        <p>
          The SN3 state generalizes this to <em>hyperedges</em>: a hyperedge is a list of one or
          more vertices, possibly repeating a vertex. Unlike an ordinary edge it carries no
          relative minus sign, and repeating a vertex adds to it again, so a hyperedge spanning
          vertices <span className="math">v₁, …, vₖ</span> carries the unnormalized vector
        </p>
        <div className="equation">|c⟩ = Σᵢ |vᵢ⟩</div>
        <p>
          The generalized grid state is the trace-normalized sum of{" "}
          <span className="math">|c⟩⟨c|</span> over every hyperedge — not divided by the number
          of hyperedges directly, so a hyperedge spanning more vertices, or a vertex reused across
        ...
        <ul>
          ...
          <li>
            ...
            <span className="math">|ij⟩ + |kl⟩</span> coherence (no minus sign, unlike a plain
            grid edge); and
          </li>
          <li>
            one three-vertex hyperedge{" "}
            <span className="math">{"{(0,2), (1,1), (2,0)}"}</span>, carrying{" "}
            <span className="math">|02⟩ + |11⟩ + |20⟩</span> — the only hyperedge spanning three
            vertices.
          </li>
        </ul>
      </div>
...
rho = sn3_grid_state()  # 25x25 density matrix on C⁵ ⊗ C⁵`}
      ...
      <div className="doc-section">
        <h2>Try it</h2>
        <p>
          <code>sn3GridState()</code> has no free parameter; instead, pick one of its 13
          hyperedges to see the (unnormalized) component vector it contributes to the mixture.
        </p>
      ...
      <div className="doc-section">
        ...
        <p>
          ...
          <span className="badge">Schmidt number 3</span>{" "}
          <span className="badge">C⁵ ⊗ C⁵</span> <span className="badge">25×25</span>
        </p>
        <ul>
          ...
          <li>
            ...
            Result 1 / Appendix B: preparing it from a product state requires operations of
            Schmidt rank at least 3, even though it has positive partial transpose.
          </li>
        </ul>
      </div>
      ...
      <div className="doc-section">
        ...
        <p className="doc-cite">
          ...
          entangled states", Result 1 / Appendix B.{" "}
