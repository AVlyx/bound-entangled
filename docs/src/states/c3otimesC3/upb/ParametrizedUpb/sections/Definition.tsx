  ...
  return (
    <DefinitionSection>
      ...
      <EquationBlock>
        ...
        <EquationLine>
          {"|v_2\\rangle = \\cos\\theta\\,|0\\rangle + \\sin\\theta\\,|2\\rangle"}
        </EquationLine>
        <EquationLine>
          {
      ...
      <p>
        Alice's and Bob's vectors are then paired in a shifted order — |ψᵢ⟩ = |aᵢ⟩ ⊗ |b_σ(i)⟩ with σ
        = (1, 3, 0, 2, 4) — rather than index by index. This mismatch is what forces the resulting
        basis to be unextendible: every pair of the five |ψᵢ⟩ is orthogonal, either because Alice's
        factors are orthogonal or because Bob's are, covering all ten pairs (the orthogonality graph
        is the same for every UPB on <Latex>{`\\mathbb{C}^3 \\otimes \\mathbb{C}^3`}</Latex>). As
        for the Tiles and Pyramid UPBs, the bound entangled state is the uniform mixture over the
        4-dimensional orthogonal complement of the five |ψᵢ⟩:
      </p>
      ...
      <p>
        At φ_A = φ_B = 0 and θ_A = θ_B = γ_A = γ_B = arccos((√5 − 1)/2) this reduces to the Pyramid
        UPB; at φ_A = φ_B = 0 and θ_A = θ_B = γ_A = γ_B = 3π/4 it reduces to the Tiles UPB.
      </p>
    </DefinitionSection>
  );
}
